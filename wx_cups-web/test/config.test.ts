...
test('从直接环境变量读取微信客服与 cups-web 凭据', () => {
  ...
  assert.deepEqual(config.cupsCredentialsByExternalUser.get('wmAlice'), { username: 'wecom-gateway', password: 'cups-password' });
  assert.deepEqual(config.cupsCredentialsByExternalUser.get('wmBob'), { username: 'wecom-gateway', password: 'cups-password' });
...
test('为每个微信白名单用户读取独立 cups-web 凭据', () => {
  const config = loadConfig({
    CUPS_WEB_URL: 'http://127.0.0.1:8080',
    WECOM_CUPS_USER_CREDENTIALS: '{"wmAlice":{"username":"alice","password":"alice-password"},"wmBob":{"username":"bob","password":"bob-password"}}',
  ...
  assert.deepEqual(config.cupsCredentialsByExternalUser.get('wmAlice'), { username: 'alice', password: 'alice-password' });
  assert.deepEqual(config.cupsCredentialsByExternalUser.get('wmBob'), { username: 'bob', password: 'bob-password' });
});

test('拒绝缺少微信白名单用户的 cups-web 凭据映射', () => {
  assert.throws(() => loadConfig({
    CUPS_WEB_URL: 'http://127.0.0.1:8080', WECOM_CUPS_USER_CREDENTIALS: '{"wmAlice":{"username":"alice","password":"alice-password"}}',
    PRINTER_URI: 'http://127.0.0.1:631/printers/Office_A4', WECOM_CORP_ID: 'ww123', WECOM_KF_SECRET: 'kf-secret',
    WECOM_CALLBACK_TOKEN: 'callback-token', WECOM_CALLBACK_ENCODING_AES_KEY: 'abcdefghijklmnopqrstuvwxyz0123456789ABCDEFG',
    WECOM_OPEN_KF_IDS: 'wk123', WECOM_ALLOWED_EXTERNAL_USERS: 'wmAlice,wmBob',
  }), /缺少白名单用户 wmBob/);
